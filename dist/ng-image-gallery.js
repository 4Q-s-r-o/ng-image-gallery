	...
	function($rootScope, $timeout, $q, ngImageGalleryOpts){
		return {
			...
			template : 	'<div class="ng-image-gallery img-move-dir-{{_imgMoveDirection}}" ng-class="{inline:inline}" ng-hide="images.length == 0">'+
				...
				' track by image.id" ng-if="_activeImg == image" data-image-url="{{image.url}}" data-default-image="{{defaultImage}}" ondragstart="return false;" ng-attr-alt="{{image.alt || undefined}}"/>'+
										'</div>'+
